...
@AllArgsConstructor
@Slf4j
public class BioSampleDao implements DaoInterface {
    ...
    public int insert(String accession) {
        ...
        try {
            result = jdbcTemplate.update("insert into biosample(accession) values(?)", accession);
        } catch(Exception e) {
    ...
    public int[] bulkInsert(List<Object[]> accessionList) {
        ...
        try {
            results = jdbcTemplate.batchUpdate(
                    "insert into biosample(accession) values(?)",
