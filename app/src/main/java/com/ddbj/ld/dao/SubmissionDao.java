...
@AllArgsConstructor
@Slf4j
public class SubmissionDao implements DaoInterface {
    ...
    public int insert(String accession) {
        ...
        try {
            result = jdbcTemplate.update("insert into submission(accession) values(?)", accession);
        } catch(Exception e) {
    ...
    public int[] bulkInsert(List<Object[]> accessionList) {
        ...
        try {
            results = jdbcTemplate.batchUpdate(
                    "insert into submission(accession) values(?)",
