...
@AllArgsConstructor
@Slf4j
public class BioProjectDao implements DaoInterface {
    ...
    public int insert(String accession) {
        ...
        try {
            result = jdbcTemplate.update("insert into bioproject(accession) values(?)", accession);
        } catch(Exception e) {
    ...
    public int[] bulkInsert(List<Object[]> accessionList) {
        ...
        try {
            results = jdbcTemplate.batchUpdate(
                    "insert into bioproject(accession) values(?)",
