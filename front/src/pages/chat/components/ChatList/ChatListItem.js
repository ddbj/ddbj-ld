        ...
        return (
                    ...
                    {!chat.isGroup ?
                        ...
                        : <ul className={s.avatarsColumn}>
                            {this.chatUsers().map(user => (
                                <li key={uuid()}><Avatar user={user} size={35} showStatus={false} className="mr-3"
                                                         stroke={true}/></li>
                            ))}
                        </ul>
                    }
                    ...
                    <section className={`${s.chatItemMain} ml-3`}>
                        <header className="d-flex align-items-center justify-content-between mb-1">
                            <h6 className={`${s.chatTitle}`}>
                                {chat.isGroup ?
                                    <img alt="group" className={`${s.groupChatIcon} mr-1`} src={people}/> : null}
                                {chat.title}
            ...
            {this.time()}
                        ...
                        <p className={`${s.chatLastMessage}`}>
                            ...
                            {chat.lastMessage.owner && chat.isGroup ? <span
                                className={`${s.ownerIndicator} mr-1`}>{this.findUser(chat.lastMessage.userId)}</span> : ''}
