...
const RawDataFile = ({match}) => {
    ...
    const uploadedFiles = useUploadedFiles(id, 'raw_data_file')
    const projectBook = useProjectBook(id)

    const rows = useMemo(() => {
        if (!projectBook) return null
        const rows = getResourceService.getRawDataFileList(projectBook)
    ...
    return (
        <Widget>
            <div className="d-flex align-items-center w-100 mb-4">
                <h3 className="page-title flex-grow-1 mb-0">生データのアップロード</h3>
            </div>
