...
import ConditionMsList from '../../../../components/List/ConditionMs'
...
const List = ({match}) => {
    ...
    const conditionMsList = useMemo(() => {
        ...
        const rows = (measrumentMethod.ms_condition_id || []).map(conditionMsId => getResourceService.getConditionMs(projectBook, conditionMsId))
        return viewCoreService.getDataList(projectBook, rows, viewParseService.parseConditionMs)
    }, [measurementMethodId, measurementMethodType, projectBook])

    return <ConditionMsList conditionMsList={conditionMsList}/>
}
