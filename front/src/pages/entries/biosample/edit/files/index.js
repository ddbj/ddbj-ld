...
const Files = ({match, location, history}) => {
    ...
    return (
        <>
            ...
            <Switch>
                <Route path={"/entries/biosample/:uuid/files/upload"} component={Upload}/>
                <Route path={"/entries/biosample/:uuid/files/download"} component={Download}/>
                <Redirect path="*" to={`/entries/biosample/${uuid}/files/upload`}/>
            </Switch>
        </>
    );
}
